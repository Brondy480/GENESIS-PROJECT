...
import { DollarSign, FolderOpen, Handshake, Inbox, FileText } from "lucide-react";
...
export default function CreatorHome() {
  ...
  useEffect(() => {
    getCreatorDashboard()
      .then(r => {
        // Handle both r.data and r directly (depending on API wrapper)
        const dashboardData = r.data || r;
        setData({
          totalFundsRaised: dashboardData.totalFundsRaised || 0,
          totalProjects: dashboardData.totalProjects || 0,
          activeDeals: dashboardData.activeDeals || 0,
      ...
      .catch(err => {
        ...
        // Keep default empty data instead of fake data
      })
  ...
  const stats = [
    { label: "Total Raised", value: `${(data?.totalFundsRaised || 0).toLocaleString()} FCFA`, Icon: DollarSign, color: "#059669", tag: "+12%", tagBg: "#D1FAE5", tagColor: "#059669" },
    { label: "Active Projects", value: data?.totalProjects || 0, Icon: FolderOpen, color: "#7C3AED", tag: "+1", tagBg: "#D1FAE5", tagColor: "#059669" },
    { label: "Active Deals", value: data?.activeDeals || 0, Icon: Handshake, color: "#2563EB", tag: "Live", tagBg: "#DBEAFE", tagColor: "#2563EB" },
    { label: "Pending Requests", value: data?.pendingRequests || 0, Icon: Inbox, color: "#D97706", tag: "Review", tagBg: "#FEF3C7", tagColor: "#D97706" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#7C3AED,#4C1D95)", borderRadius: 24, padding: "28px 32px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: "linear-gradient(135deg,#C4B5FD,#7C3AED)", border: "2px solid rgba(255,255,255,0.4)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
              {user?.profileImage
                ? <img src={user.profileImage} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
                : <span style={{ color: "white", fontFamily: F.jakarta, fontWeight: 800, fontSize: 20 }}>{user?.name?.[0]?.toUpperCase() || "C"}</span>}
            </div>
            <div>
        ...
        {stats.map((s, i) => (
          <div key={i} style={{ ...card, transition: "all 0.3s" }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: "#F5F3FF", display: "flex", alignItems: "center", justifyContent: "center" }}><s.Icon size={22} color={s.color} /></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 16 }}>
        ...
        <div style={{ ...card, padding: 0 }}>
          ...
          {(data?.recentProjects || []).length === 0 ? (
            <div style={{ padding: "48px 20px", textAlign: "center" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}><FolderOpen size={40} color="#DDD6FE" /></div>
            ...
            return (
              <div key={i} style={{ padding: "14px 20px", borderBottom: "1px solid rgba(124,58,237,0.05)", display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: "linear-gradient(135deg,#EDE9FE,#DDD6FE)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><FolderOpen size={20} color="#7C3AED" /></div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ fontFamily: F.dm, fontSize: 11, color: "rgba(13,6,33,0.4)" }}>{(p.totalFunded || 0).toLocaleString()} FCFA raised · {pct}%</span>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ ...card, padding: 0 }}>
          ...
          {(data?.recentRequests || []).length === 0 ? (
            <div style={{ padding: "48px 20px", textAlign: "center" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}><Inbox size={40} color="#DDD6FE" /></div>
            ...
            <div key={i} style={{ padding: "14px 20px", borderBottom: "1px solid rgba(124,58,237,0.05)" }}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontFamily: F.dm, fontSize: 12, color: "rgba(13,6,33,0.5)", marginBottom: 8 }}>{(r.amount || 0).toLocaleString()} FCFA · {r.equityRequested}% equity</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
        {[
          { Icon: FolderOpen, label: "Post New Project", desc: "Start raising capital", bg: "#EDE9FE", color: "#7C3AED", action: () => navigate("/creator/dashboard/projects") },
          { Icon: Inbox, label: "Review Requests", desc: `${data?.pendingRequests || 0} pending`, bg: "#FEF3C7", color: "#D97706", action: () => navigate("/creator/dashboard/requests") },
          { Icon: FileText, label: "Sign Agreements", desc: "Complete your deals", bg: "#DBEAFE", color: "#2563EB", action: () => navigate("/creator/dashboard/agreements") },
          { Icon: DollarSign, label: "View Wallet", desc: `${(data?.walletBalance || 0).toLocaleString()} FCFA balance`, bg: "#D1FAE5", color: "#059669", action: () => navigate("/creator/dashboard/wallet") },
        ].map((a, i) => (
          <button key={i} onClick={a.action} style={{
            ...
            onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 12px 40px rgba(124,58,237,0.12)"; }}
            onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 2px 16px rgba(124,58,237,0.06)"; }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: a.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><a.Icon size={22} color={a.color} /></div>
            <div>
