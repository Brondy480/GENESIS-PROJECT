...
import {
  LayoutDashboard, FolderOpen, Inbox, Handshake, FileText,
  Wallet, User, Globe, Bell, LogOut, DollarSign,
} from "lucide-react";
...
const FEED_NAV = [
  { path: "/feed", label: "Discover Projects", Icon: Globe, highlight: true },
];
...
export default function CreatorLayout() {
  ...
  const markAllRead = async () => {
    try { await markAllNotificationsRead(); setUnreadCount(0); setNotifications(p => p.map(n => ({ ...n, isRead: true }))); } catch { /* ignore */ }
  };
      ...
      <aside style={{
          ...
          {FEED_NAV.map(item => (
            <a
              ...
              style={{
                ...
                color: "#E9D5FF",
                fontFamily: F.jakarta, fontWeight: 600, fontSize: 14,
              }}
            >
              <span style={{
                ...
                transition: "all 0.2s",
              }}><item.Icon size={16} color="#E9D5FF" /></span>
            ...
            >
              {({ isActive }) => (<>
                <span style={{
                  ...
                  transition: "all 0.2s",
                }}><item.Icon size={16} color={isActive ? "#7C3AED" : "rgba(255,255,255,0.7)"} /></span>
                {!collapsed && <span style={{ whiteSpace: "nowrap" }}>{item.label}</span>}
              </>)}
            </NavLink>
          ))}
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 12, marginTop: 8 }}>
          <button onClick={() => { logout(); navigate("/login"); }} style={{
            ...
            <span style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><LogOut size={16} color="rgba(255,255,255,0.4)" /></span>
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              }}><Bell size={18} color="#7C3AED" /></button>
              {unreadCount > 0 && (
                <div style={{
              ...
              {showNotifs && (
                <div style={{
                  position: "absolute", right: 0, top: 48, width: 340, background: "white",
                  borderRadius: 18, boxShadow: "0 20px 60px rgba(0,0,0,0.12)",
                  border: "1px solid rgba(124,58,237,0.1)", zIndex: 200, overflow: "hidden"
                }}>
                  <div style={{
                    padding: "14px 18px", borderBottom: "1px solid rgba(124,58,237,0.07)",
                  ...
                  <div style={{ maxHeight: 300, overflowY: "auto" }}>
                    {notifications.length === 0 ? (
                      <div style={{ padding: "32px", textAlign: "center" }}>
                        <div style={{ display: "flex", justifyContent: "center", marginBottom: 8 }}><Bell size={32} color="rgba(13,6,33,0.2)" /></div>
                        <p style={{ fontFamily: F.dm, fontSize: 13, color: "rgba(13,6,33,0.4)" }}>No notifications yet</p>
                      </div>
                    ) : notifications.map((n, i) => (
                      <div key={i} style={{
                        padding: "12px 18px", borderBottom: "1px solid rgba(124,58,237,0.05)",
                        ...
                        <div style={{ width: 34, height: 34, borderRadius: 9, background: "#EDE9FE", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          {n.type === "agreement_signing_required"
            ...
            <button onClick={() => navigate("/creator/dashboard/profile")} style={{
              ...
              <div style={{
                width: 38, height: 38, borderRadius: 12, background: "linear-gradient(135deg,#7C3AED,#A78BFF)",
